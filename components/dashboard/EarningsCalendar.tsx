...
export default function EarningsCalendar() {
  ...
  if (!isLoading && earnings.length === 0 && allHoldings.length === 0) return null;
  ...
  return (
    <div className="glass-card p-6 animate-fade-in-up opacity-0 stagger-3">
      {/* Header */}
      ...
      {!isLoading && earnings.length === 0 && allHoldings.length > 0 && (
        <p className="text-sm text-muted text-center py-4">ไม่พบข้อมูล Earnings ในช่วงนี้</p>
      )}
    </div>
  );
}
